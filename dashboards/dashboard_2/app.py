...
if use_defaults:
    ...
    latitude = st.sidebar.number_input(
        "Latitude", min_value=-90.0, max_value=90.0, value=51.75, step=0.01
    )
    longitude = st.sidebar.number_input(
        "Longitude", min_value=-180.0, max_value=180.0, value=-1.25, step=0.01
    )
    capacity_kwp = st.sidebar.number_input("Capacity (kWp)", min_value=0.1, value=1.25, step=0.01)

inverter_type = st.sidebar.selectbox(
    "Select Inverter", ["No Inverter", "Enphase", "Solis", "GivEnergy", "Solarman"]
)
...
if inverter_type == "Enphase" and not st.session_state.enphase_access_token:
    ...
    if auth_url_response:
        ...
        st.write(
            ...
            "Please copy the entire URL and paste it below:"
        )

        st.session_state.redirect_url = st.text_input(
            "Enter the redirect URL:", value=st.session_state.redirect_url
        )
            ...
            if not st.session_state.redirect_url:
                ...
                try:
                    token_response = make_api_request(
                        ...
                        method="POST",
                        data={"redirect_url": st.session_state.redirect_url},
                    )
                    if token_response:
        ...
        if inverter_type == "No Inverter":
            if "power_kw" in predictions.columns:
                fig = px.line(
                    ...
                    labels={"power_kw": "Forecast", "index": "Time"},
                )
            else:
                st.error(
                    "Expected column 'power_kw' not found in the data for No Inverter scenario."
                )
                st.stop()
        else:
            if "power_kw" in predictions.columns and "power_kw_no_live_pv" in predictions.columns:
                ...
                st.error(
                    "Expected columns 'power_kw' and 'power_kw_no_live_pv' not found "
                    "in the data for Inverter scenario."
                )
                st.stop()
