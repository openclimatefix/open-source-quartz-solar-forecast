...
def main(save_outputs: bool = False):
    ...
    timestamp_str = datetime.fromtimestamp(timestamp, tz=UTC).strftime("%Y-%m-%d %H:%M:%S")
    ts = pd.to_datetime(timestamp_str)

    # make input data with live enphase, solis, givenergy, or solarman data
    site_live = PVSite(
        latitude=51.75, longitude=-1.25, capacity_kwp=1.25, inverter_type="enphase"
    )  # inverter_type="enphase", "solis", "givenergy", "solarman" or "victron"
