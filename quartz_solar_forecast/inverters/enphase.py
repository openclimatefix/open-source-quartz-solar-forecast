...
def get_enphase_auth_url(settings: EnphaseSettings | None = None):
    """
    ...
    """
    if settings is None:
        # Because this uses env variables we don't want to set it as a default argument,
        # otherwise it will be evaluated even if the method is not called
...
def get_enphase_authorization_code(auth_url):
    """
    Open the authorization URL in a browser and retrieve the authorization code
...
def get_enphase_access_token(auth_code: str | None = None, settings: EnphaseSettings | None = None):
    """
...
def process_enphase_data(data_json: dict, start_at: int) -> pd.DataFrame:
    ...
    for interval in data_json["intervals"]:
        ...
        if end_at >= start_at:
            # Convert to UTC
            timestamp = datetime.fromtimestamp(end_at, tz=UTC).strftime("%Y-%m-%d %H:%M:%S")
...
def get_enphase_data(settings: EnphaseSettings) -> pd.DataFrame:
    ...
    conn = http.client.HTTPSConnection("api.enphaseenergy.com")
    headers = {"Authorization": f"Bearer {access_token}", "key": settings.api_key}

    # Add the system_id and duration parameters to the URL
    url = (
        f"/api/v4/systems/{settings.system_id}/telemetry/production_micro"
        f"?start_at={start_at}&granularity={granularity}"
    )
