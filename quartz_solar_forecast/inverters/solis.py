...
class SoliscloudAPI:
        ...
        def __init__(self, statuscode, message=None):
            ...
            if not message:
                if statuscode == 408:
                    ...
                    self.message = (
                        f"Your system time is different from server time, your time is {now}"
                    )
                else:
    ...
    class ApiError(SolisCloudError):
        ...
        def __str__(self):
            return (
                f"API returned an error: {self.message}, error code: {self.code}, "
    ...
    async def _get_records(
        self, canonicalized_resource: str, key_id: str, secret: bytes, params: dict[str, Any]
    ):
        """
    ...
    async def _post_data_json(
        self, url: str, header: dict[str, Any], params: dict[str, Any]
    ) -> dict[str, Any]:
...
class SolisData:
    def __init__(self, settings: SolisSettings):
        ...
        if not self.api_key or not api_secret_str:
            raise ValueError(
                "SOLIS_CLOUD_API_KEY or SOLIS_CLOUD_API_KEY_SECRET environment variable is not set"
            )
    ...
    async def get_inverter_list(self, soliscloud: SoliscloudAPI):
        ...
        inverter_list = await soliscloud.inverter_list(
            self.api_key, self.api_secret, page_no=1, page_size=100
        )
            ...
            for inverter in inverter_list:
                ...
                for day in range(7):
                    ...
                    try:
                        ...
                        if isinstance(inverter_day_data, list) and all(
                            isinstance(item, dict) for item in inverter_day_data
                        ):
                            for data_point in inverter_day_data:
                                timestamp = datetime.fromtimestamp(
                                    int(data_point["dataTimestamp"]) / 1000, tz=UTC
                                )
                                if start_time <= timestamp <= end_time:
                                    data_list.append(
                                        {
                                            "timestamp": timestamp.strftime("%Y-%m-%d %H:%M:%S"),
                                            "power_kw": float(data_point["pac"])
                            ...
                            print(
                                f"Unexpected data format for inverter {inverter_sn} "
                                f"on {current_date}"
                            )
                        ...
                        print(
                            f"Error fetching data for inverter {inverter_sn} on {current_date}: {e}"
                        )
                        print(f"Received data: {inverter_day_data}")
